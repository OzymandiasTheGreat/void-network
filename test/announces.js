...
test("announce to group and lookup", async function (t) {
	const [a, b] = await swarm(t);
	const keyPair1 = VoidDHT.keyPair();
	const keyPair2 = VoidDHT.keyPair();
	const target = VoidDHT.hash(Buffer.from("testing..."));
...

	{
		...
		t.alike(
			[
				result[0].peers[0].publicKey,
				result[0].peers[1].publicKey,
			].sort(),
